...
// Import services and models yang diperlukan
...
class _IuranPaymentScreenState extends State<IuranPaymentScreen> {
  ...
  Future<void> _fetchTagihanAndWallet() async {
    ...
    try {
      ...
      // 3. SIMULASI PEMBUATAN TAGIHAN BELUM LUNAS (Menggunakan model Iuran yang sudah ada)
      final List<UserTagihan> pendingTagihan = [];
      final random = Random();

      for (var iuran in jenisIuran.take(3)) { 
            pendingTagihan.add(
                UserTagihan(
                    id: 'TGH-${iuran.id}-${random.nextInt(1000)}',
                    iuran: iuran,
                    jumlah: iuran.jumlah,
                    // Menggunakan Jatuh Tempo dari iuran jika tersedia, jika tidak, gunakan simulasi
                    jatuhTempo: iuran.jatuhTempo ?? DateTime.now().subtract(Duration(days: random.nextInt(20))), 
                    status: 'Belum Lunas',
                ),
            );
      }
  ...
  // --- LOGIKA PEMBAYARAN (Integrasi API) ---
  Future<void> _processPayment() async {
    ...
    try {
      // 1. PANGGIL API PEMBAYARAN IURAN SEBENARNYA
      ...
      if (success && mounted) {
        ...
        // 2. Refresh SALDO dan Riwayat Transaksi via AuthProvider (WAJIB)
        ...
        // 4. Sinyal ke HomeTabWalletContent agar me-refresh riwayat transaksi (WAJIB)
        ...
        if (e.response?.data != null && e.response!.data.containsKey('message')) {
             ...
             errorMessage = "Gagal validasi data. Saldo mungkin kurang atau ID tagihan salah.";
        }
          ...
          child: Column(
            ...
            children: [
              Padding(
                ...
                child: Text(
                  "Tagihan Belum Lunas (${_tagihanList.length})",
