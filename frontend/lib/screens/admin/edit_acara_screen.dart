...
class EditAcaraScreen extends StatefulWidget {
  ...
  @override
  State<EditAcaraScreen> createState() => _EditAcaraScreenState();
}

class _EditAcaraScreenState extends State<EditAcaraScreen> {
  ...
  void initState() {
    ...
    _namaController = TextEditingController(text: acara.namaAcara);
    _deskripsiController = TextEditingController(text: acara.deskripsi);
    ...
    // Inisialisasi Biaya (tanpa koma/titik)
    ...
    _waktuMulai = TimeOfDay.fromDateTime(acara.tanggalMulai);
    _tanggalSelesai = acara.tanggalSelesai;
    _waktuSelesai = TimeOfDay.fromDateTime(acara.tanggalSelesai);
  }
  ...
  Future<void> _submitUpdateAcara() async {
    ...
    try {
      final success = await apiService.updateAcara(widget.acara.id, data);
      if (success && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            child: Text(
              "Edit Acara: ${widget.acara.namaAcara}",
  ...
  Widget _buildSectionTitle(BuildContext context, String title) {
    return Padding(
      // Padding atas dikurangi menjadi 4.0
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // Field Biaya (Sudah Disesuaikan)
                    TextFormField(
                      controller: _biayaController,
                      decoration: _inputDecoration.copyWith(
                        labelText: "Total Biaya (Rp)",
                        // Penyesuaian Prefix untuk memberi jarak
                        prefixText: 'Rp ',
                        prefixStyle: const TextStyle(fontWeight: FontWeight.bold, color: _primaryColor),
                        // Penyesuaian contentPadding (tetap 16 semua sisi)
                      ),
                    ...
                    // --- Tombol SIMPAN ---
                    ElevatedButton(
                      onPressed: _isLoading ? null : _submitUpdateAcara,
